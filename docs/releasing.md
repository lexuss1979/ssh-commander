...
6. Для повтора без изменения скриптов используйте **Re-run failed jobs**. Если исправлены сами скрипты выпуска, выберите **Run workflow → main** и явно укажите `release_tag` существующего стабильного тега. Workflow разрешает его в SHA, повторяет проверки именно этого SHA и собирает приложение в отдельном checkout; скрипты оркестрации из `main` в образ не попадают. Без тега ручной запуск из ветки ничего не публикует. Скрипт использует уже опубликованную версию или кандидата того же SHA и проверяет OCI revision/version на обеих платформах. Ошибка сети/авторизации останавливает выпуск; только подтверждённый registry 404 означает отсутствие manifest. Если кандидат имеет неверные метаданные, не удалять/переписывать его автоматически: выяснить причину и выбрать новый выпуск.
