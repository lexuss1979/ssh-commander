...
- `buildDuCommand(path)` →
  ...
  **Отклонение от текста roadmap (`-B1`): `-k`, а не `-B1`.** BusyBox `du`
  ...
  одной ФС (тот же смысл, что в roadmap); `-d 1` — только прямые
  подкаталоги, `.` не печатается (GNU) / печатается как сам путь (BusyBox —
  парсер это покрывает, см. ниже).
...
- `needsStatFallback(stderr)` — чистая: `/unknown primary|invalid option/i`
  (GNU/BusyBox по-разному сообщают о незнакомой опции `find`) → true.
...
- `parseDuKb(text, basePath)` → `{totalKb: number | null, children:
  ...
  путь с пробелами/табами — хвост строки). Первая запись с
  `path === basePath` — сумма поддерева (у GNU du это первая строка, у
  BusyBox — строка с самим путём; несовпадение путей → `totalKb: null` —
  вызывающий отдаёт ошибку). Children — записи с путём, начинающимся с
  `basePath + '/'`. Прочее (мусор, обрезанный хвост без таба) — пропуск.
...
- `diskUsageSnapshot(profile, path)` — предпроверка SFTP stat (`withSftp`
  ...
  путь пропал между stat и du); `totalKb === null` → Error «du не вернул
  размер каталога». Ответ — `{path, totalBytes, directBytes, children,
  incomplete}` (`incomplete: {unreadable: n} | null`). Кэш
...
- `parseDuKb`: нормальный вывод, пути с пробелами, первая запись = сумма
  поддерева, children-фильтр (чужой путь → пропуск), мусорные строки,
  отсутствие total → null, обрезанный хвост без таба, BusyBox-вариант
  (путь без `./`-префикса);
- `toDuSnapshot`: сортировка по убыванию, округление pct, `directBytes`
  ...
  файлы и т.п.), пустой список детей;
