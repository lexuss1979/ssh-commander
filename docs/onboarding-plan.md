...
- `POST /api/setup` — тело `{ password, aiApiKey?, aiApiBase? }`:
  ...
  - валидация: `password` — непустая, длина ≥ 8 (см. «Решения»), без `\n`/`\r`;
...
- `web/src/pages/OnboardingPage.tsx` — экран вместо LoginPage:
  ...
  - «Ключ API» (опционально) + селект провайдера (OpenAI / DeepSeek / свой
    URL) → поле «Base URL» при «свой URL» (см. «Решения»);
...
- Ключ/пароль **нельзя сменить или добавить позже через UI** — осознанное
  ограничение v1 (следующий эпик «Настройки»: `GET` с маскированным ключом +
  `PUT`); до него — только правка `data/settings.json` или env+restart.
...
1. Провайдер в onboarding: селект «OpenAI / DeepSeek / свой URL» или просто
   поле ключа + опциональное поле Base URL?
2. Минимальная длина пароля: ≥ 8 (рекомендую — сейчас ограничений нет) или
   без ограничения?
3. Авто-вход сразу после setup (рекомендую да) или возврат на страницу логина?
4. Эпик «Настройки» (смена пароля/ключа в UI) — планировать сразу следом
   или позже, по спросу?
