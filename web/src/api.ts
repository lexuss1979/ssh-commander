...
export function fetchDbInstances(
  profileId: string,
): Promise<{ instances: DbInstanceInfo[]; hints: DbHint[] }> {
  return api(`/api/db/instances?profileId=${encodeURIComponent(profileId)}`);
}
...
export function fetchDbOverview(profileId: string, instanceId: string): Promise<DbOverview> {
  const params = new URLSearchParams({ profileId, instanceId });
...
export function fetchDbTables(
  ...
  const params = new URLSearchParams({ profileId, instanceId, database });
...
export async function runDbQuery(
  profileId: string,
  params: { instanceId: string; database: string; sql: string; readOnly: boolean },
