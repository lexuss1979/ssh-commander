...
export function DatabasesPage({
  ...
  // Инстансы (обновление по кнопке — polling нет, серверный кэш 2 с)
  const [instances, setInstances] = useState<DbInstanceInfo[] | null>(null);
  const [hints, setHints] = useState<DbHint[]>([]);
  const [instancesError, setInstancesError] = useState<string | null>(null);
  const [reloadKey, setReloadKey] = useState(0);

  // Выбор: инстанс → база → таблицы/колонки
  ...
  /** Счётчик контекста: инкремент при смене инстанса/базы — ответ
  ...
  // Загрузка списка инстансов при первом показе вкладки и по кнопке.
  useEffect(() => {
    if (!visible) return;
    let cancelled = false;
    setInstancesError(null);
    fetchDbInstances(profile.id)
      .then((res) => {
        if (cancelled) return;
      ...
      .catch((err) => {
        if (!cancelled) setInstancesError((err as Error).message);
      });
    return () => {
      cancelled = true;
    };
  ...
  // Сменился инстанс — тащим обзор (версия, базы с размерами). Прошлые
  // база/таблицы/результаты недействительны сразу: без сброса database
  // таблицы успели бы запроситься у нового инстанса со старой базой.
  useEffect(() => {
    ...
    setResult(null);
    setQueryError(null);
    fetchDbOverview(profile.id, instanceId)
      .then((res) => {
        if (cancelled) return;
        setOverview(res);
  ...
  useEffect(() => {
    ...
    setColumns(null);
    setColumnsTruncated(false);
    fetchDbTables(profile.id, instanceId, database)
      .then((res) => {
    ...
    fetchDbColumns(profile.id, instanceId, database)
      .then((res) => {
        if (!cancelled) {
  ...
  }, [profile.id, instanceId, database, showError]);
  ...
  const runQuery = useCallback(async () => {
    if (!instance || !database) {
      showError('Выберите инстанс и базу');
      return;
    }
    if (!sql.trim() || running) return;
    // Поколение контекста: если инстанс/база сменились, пока шёл запрос
  ...
  }, [instance, database, sql, running, readOnly, profile.id, showError, pushHistory]);

  const handleDump = async (dbName: string) => {
    if (!instance || dumping) return;
    setDumping(dbName);
    try {
      await downloadDbDump(profile.id, instance.id, dbName);
    } catch (err) {
  ...
  const insertExplain = () => {
    ...
    setSql((prev) => {
      ...
      const prefix = instance.engine === 'postgres'
  ...
  const prefillTable = (t: DbTableInfo) => {
    const quote = (s: string) => (instance?.engine === 'postgres' ? `"${s}"` : `\`${s}\``);
  ...
  const askAgent = () => {
    ...
    const engineLabel = ENGINE_LABEL[instance.engine] ?? instance.engine;
  ...
  return (
    ...
    >
      <div className="toolbar">
        <span className={`status-dot ${instancesError ? 'error' : instances ? 'connected' : ''}`} />
        <span className="status-text">
          {instancesError
            ? `Ошибка: ${instancesError}`
            : instances
              ? `${instances.length} инстанс(ов)`
              : 'Загрузка…'}
        </span>
      ...
      {instancesError && !instances ? (
        <div className="empty-state">
          <p>Docker недоступен или список инстансов получить не удалось:</p>
          <p className="error-text">{instancesError}</p>
        ...
        <div className="db-layout">
          <aside className="db-sidebar">
            <h3 className="section-title">Инстансы</h3>
            {instances.length === 0 && (
              <p className="muted db-sidebar-empty">
            ...
            {instances.map((i) => (
              <button
                key={i.id}
                type="button"
                className={`db-item${i.id === instanceId ? ' selected' : ''}`}
                onClick={() => setInstanceId(i.id)}
                title={i.image}
              >
                <span className={`db-engine-badge ${i.engine}`}>
                  {i.engine === 'postgres' ? 'PG' : 'MY'}
                </span>
                <span className="db-item-body">
                  <strong>{i.name}</strong>
                  <span className="muted">{ENGINE_LABEL[i.engine] ?? i.engine}</span>
                </span>
              </button>
            ))}
              ...
              <div className="db-hints">
                {hints.map((h) => (
                  <p key={h.id} className="muted" title={`Контейнер ${h.name}, порт ${h.port}`}>
                    ...
                    (в v1 поддерживаются postgres / mysql / mariadb)
                  </p>
                ))}
              </div>
            )}
          ...
          <section className="db-main">
            <div className="db-editor-toolbar">
              ...
              <button
                ...
                onClick={runQuery}
                disabled={running || !instance || !database}
              >
