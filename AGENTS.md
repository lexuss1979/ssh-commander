...
- `src/services/ports.ts` + `src/routes/ports.ts` — прослушиваемые порты для вкладки «Порты» (`GET /api/ports?profileId=`): `ss -tulpn` по SSH (фолбэк `netstat -tulpn`), чистый парсер `parseListeners` (определяет формат по строке, вычисляет scope public/loopback/interface), кэш 2 с на профиль. Без root имена чужих процессов недоступны — колонка процесса пустая. `src/services/container-ports.ts` — порты docker-контейнеров: `docker ps` + батч-`docker inspect`, парсинг `NetworkSettings.Ports` (runtime-биндинги, не строковый `Ports` из `docker ps`), кэш 2 с. Роут собирает хостовые и контейнерные порты параллельно (`Promise.allSettled`); docker недоступен → поля `containers` нет (деградация); хостовые слушатели матчатся с опубликованными контейнерными портами по `(proto, hostPort)` — аннотация `container: {id, name}` закрывает дыру «безымянный docker-proxy без root».
