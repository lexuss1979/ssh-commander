...
- CRUD `/api/profiles`, `POST /api/profiles/:id/reconnect` (закрывает SSH-подключение профиля; следующее обращение откроет свежее — применяются новые группы/права), `GET /api/keys` (список имён), `POST /api/keys?name=&overwrite=1` (импорт ключа из UI, raw body)
