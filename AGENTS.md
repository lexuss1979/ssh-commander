...
- `src/services/overview.ts` + `src/routes/overview.ts` — сводный дашборд всех профилей (`GET /api/overview`, без profileId): параллельный опрос (Promise.allSettled), guard-таймаут 8 с на профиль (упавший сервер → `ok:false` с ошибкой, ответ не висит), docker-счётчики через `docker ps -a --format json` (при недоступном docker — просто нет поля), кэш всего ответа 4 с. Агрегирующий маппинг — чистые функции `toOverviewEntry`/`countContainers`.
...
- `GET /api/overview` — сводный снимок по всем профилям (вкладка «Серверы»): `{timestamp, servers: [{id, name, host, port, username, ok, error?, metrics?, docker?: {containersTotal, containersRunning}}]}`.
...
- Read-only (выполняются автоматически): `exec_readonly, read_file, read_memory, list_dir, docker_ps, docker_logs, docker_inspect, security_audit`.
...
- `cd server && npm test` — unit-тесты: `guard.test.ts` (deny-лист), `path.test.ts` (безопасность путей), `docker-json.test.ts` (парсер вывода docker), `docker-run.test.ts` (сборка `docker run` с `sh -c`), `docker-ops.test.ts` (prune/compose-аргументы, парсинг `stats`), `dialogues.test.ts` и `profiles.test.ts` (хранилища, частичный update секретов), `corrupt-store.test.ts` (битый JSON: `*.corrupt-*` и отказ persist), `metrics.test.ts` (парсинг метрик), `ports.test.ts` (парсер ss/netstat: процессы, scope, отсутствие прав), `container-ports.test.ts` (порты контейнеров: парсинг `NetworkSettings.Ports` — опубликованные/exposed/host-network/interface-specific/multi-binding, маппинг `toContainerPortEntry`, аннотация хостовых слушателей по `(proto, hostPort, hostIp)`), `tunnels.test.ts` (SSH-туннели: валидация параметров — порты, localHost, charset targetHost, границы диапазона, дубли, лимит на профиль — только active, closed не считаются), `overview.test.ts` (сводный дашборд: маппинг allSettled-результатов, счётчики контейнеров), `file-search.test.ts` (команды и парсинг поиска), `transfer.test.ts` (tar-команды, детект отсутствия tar), `history.test.ts` (парсинг истории bash/zsh, дедуп), `plan.test.ts` (режим планирования), `messages.test.ts` (санитизация истории сообщений агента), `memory.test.ts` (память агента: чтение/запись и безопасность путей MEMORY.md), `keys.test.ts` (импорт ключей: санитизация имени, валидация PEM, права 0600, защита от затирания), `profile-transfer.test.ts` (перенос профилей: шифрованный/открытый round-trip, неверный пароль, подмена шифротекста, отказ от затирания ключей, валидация бэкапа), `security-audit.test.ts` (аудит безопасности: билдеры команд по секциям, root-only пометки, sudo-обёртка без пароля в командной строке, деградация без пароля/sudo, лимиты вывода, docker-флаги).
