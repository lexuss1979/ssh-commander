...
- `src/services/ports.ts` + `src/routes/ports.ts` — прослушиваемые порты для вкладки «Порты» (`GET /api/ports?profileId=`): `ss -tulpn` по SSH (фолбэк `netstat -tulpn`), чистый парсер `parseListeners` (определяет формат по строке, вычисляет scope public/loopback/interface), кэш 2 с на профиль. Без root имена чужих процессов недоступны — колонка процесса пустая.
...
- `GET /api/ports?profileId=` — прослушиваемые порты (вкладка «Порты»): `{timestamp, ports: [{proto, host, port, pid, process, scope}]}`.
